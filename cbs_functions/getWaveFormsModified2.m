function wf = getWaveFormsModified2(gwfparams)
% function wf = getWaveFormsModified(gwfparams)
%
...
%
% Contributed by C. Schoonover and A. Fink, modified by (anonymized)
%
% % EXAMPLE INPUT
...
for curUnitInd=1:numUnits
    ...
    for curSpikeTime = 1:min([gwfparams.nWf curUnitnSpikes])
        ...
        t_ind = spikeTimeKeeps(curUnitInd, curSpikeTime) + gwfparams.wfWin(1):spikeTimeKeeps(curUnitInd, curSpikeTime) + gwfparams.wfWin(end);
        t_ind = min(t_ind, nSamp);
        t_ind = max(t_ind, 1);
            
        tmpWf = mmf.Data.x(1:gwfparams.nCh, t_ind);
